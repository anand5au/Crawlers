 ...
 * @option String nextId The element to focus whe hitting enter or tab
 */
...
$.Autocompleter = function(input, options) {
			...
			case KEY.RETURN:
				if( selectCurrent() ){
					...
					//if( !options.multiple )
						//$input.blur();
						if(options.nextId != "" && 
							document != null && document.getElementById(options.nextId) != null ) {
									document.getElementById(options.nextId).focus(); 
							}
					event.preventDefault();
				}
				break;
	...
	function checkIt(string)
	{
		...
		place = version.indexOf(string) + 1;
		return place;
	}
            ...
            if(options.scroll) {
                ...
                if(navigator.appName == 'Microsoft Internet Explorer' && typeof document.body.style.maxHeight === "undefined") {
					...
					if (!scrollbarsVisible) {
						...
						listItems.width( list.width() - parseInt(listItems.css("paddidata-ng-left")) - parseInt(listItems.css("paddidata-ng-right")) );
					}
                }
